...
private:
	...
	node* root;
	int BDtoken = 2147483647; //max a 32-bit int can be
...
public:
	...
	node* Root() { // you can't call it root() as it thinks that you're calling root declared under the private section
		return root;
	}
	...
	void deletePerson(string FN, string LN) {
		...
		if (noChildren(targetNode)) {
			...
			if (index->right == NULL) {
				...
				targetNode->data = a; 
				//delete index and what it points to
	...
	//these searches dont work (/* */)
	/*void BDSearch(node* n) {
		if (n->right != NULL) {
			BDSearch(n = n->right);
		}
		if (n->left != NULL) {
	...
	int BDSearchL(node* n, int bd) { //searches for smallest BD int, or 'oldest' birthday (chekcs left then right nodes)
		if (n->left != NULL) {
			bd = BDSearchL(n = n->left, bd);
		}
		if (n->right != NULL) {
			bd = BDSearchL(n = n->right, bd);
		}
		if (bd > n->data.BD) {
			return n->data.BD;
		}
	...
	int BDSearchR(node* n, int bd) { //searches for smallest BD int, or 'oldest' birthday (chekcs right then left nodes)
		if (n->right != NULL) {
			bd = BDSearchR(n = n->right, bd);
		}
		if (n->left != NULL) {
			bd = BDSearchR(n = n->left, bd);
		}
		if (bd > n->data.BD) {
			return n->data.BD;
		}
	...
	person* BDpSearch(node* n, int bd, person* p) { //returns the pointer to the person whith the birthday you want
		if (bd == n->data.BD) {
			return &n->data;
		}
		if (n->left != NULL) {
			p = BDpSearch(n = n->left, bd, p);
		}
		if (n->right != NULL) {
			p = BDpSearch(n = n->right, bd, p);
		}
	}*/
...
void titlePage() {
	cout << "***************************************************" << endl;
	cout << "|                                                 |" << endl;
	cout << "| A database program using a binary tree          |" << endl;
